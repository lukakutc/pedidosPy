...
VER001	LECHE PURISIMA 800G ENTERA CAJA	Unidad	400,00	8.256,20	 	3.302.480,00	No seleccionado		0,00	
VER002	VASO TERMICO *25U 240CC	Unidad	80,00	4.289,26	 	343.140,80	No seleccionado		0,00	
CAR001	TE TARAGUI  25S ENSOBRADO INTERNACIONAL	Unidad	120,00	3.214,88	 	385.785,60	No seleccionado		0,00	
CAR002	MATE COC.TARAGUI  25S SIN ENSOBRAR	Unidad	120,00	1.644,63	 	197.355,60	No seleccionado		0,00	
...
lineas.forEach((linea) => {
    ...
    const arrArticulo = linea.split('\t')
    const codArticulo = arrArticulo.at(0)
    console.log('codigo  ?  ' + codArticulo)
    const dbArticulo = db.prepare(`SELECT id_articulo,sector FROM articulo WHERE id_articulo = ?`).all(codArticulo)
    //si la consulta devolvio un articulo
    if (dbArticulo.length > 0) {
        //me fijo el sector del articulo y lo añado a la lista.
        //se encuentra en la segunda posicion (id_articulo,sector)
        const sectorArticulo = dbArticulo[0].sector
        console.log('Sector del articulo: ' + dbArticulo[0].id_articulo + ' es: ' + dbArticulo[0].sector)
        if (dbArticulo[0].sector == 'ALMACEN') {
            listaAlmacen.push(codArticulo)
        } else if (dbArticulo[0].sector == 'FIAMBRERIA')
            listaFiambreria.push(codArticulo)
        else if (dbArticulo[0].sector == 'PANADERIA')
            listaPanaderia.push(codArticulo)
        else if (dbArticulo[0].sector == 'CARNICERIA')
            listaCarniceria.push(codArticulo)
        else
            listaVerduleria.push(codArticulo)
    } else {
        //sino esta en la base de datos lo creo
        const sector = 'ALMACEN'
        console.log('cod? '+arrArticulo[posicionCod]+' desc? '+arrArticulo[posicionDesc]+' sector? '+sector)
        //aca se debe ingresar un sector de los 
        db.prepare(
